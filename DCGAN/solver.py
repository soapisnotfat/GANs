    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, images in enumerate(self.data_loader):
                ...
                real_loss = func.binary_cross_entropy(outputs, torch.ones(images.size(0), device=self.device))

                # Train D to recognize fake images as fake.
                fake_images = self.generator(noise.view(-1, 100, 1, 1))
                outputs = self.discriminator(fake_images)
                fake_loss = func.binary_cross_entropy(outputs, torch.zeros(images.size(0), device=self.device))
                ...
                fake_images = self.generator(noise.view(-1, 100, 1, 1))
                outputs = self.discriminator(fake_images)
                g_loss = func.binary_cross_entropy(outputs, torch.ones(images.size(0), device=self.device))
